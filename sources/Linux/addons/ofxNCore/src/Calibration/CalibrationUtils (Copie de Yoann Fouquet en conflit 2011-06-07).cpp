...
void CalibrationUtils::computeCameraToScreenMap()
{
	...
	for(int y = 0; y < _camHeight; y++)
	{
		for(int x = 0; x < _camWidth; x++)
		{
			...
			//transformedX*=ofGetScreenWidth();
			//transformedY*=ofGetScreenHeight();
...
void CalibrationUtils::cameraToScreenSpace(float &x, float &y)
{
	...
	//if (inCalibrationSector(x, y))
	{
        ...
        if ((t.s1->coord != pt) && (t.s2->coord != pt) && (t.s3->coord != pt))
        {
            ...
            x = ( alpha * ts.s1->_x() + beta * ts.s2->_x() + ts.s3->_x() ) / ( alpha + beta + 1 );
            y = ( alpha * ts.s1->_y() + beta * ts.s2->_y() + ts.s3->_y() ) / ( alpha + beta + 1 );
        }
        else
        {
            if (t.s1->coord == pt)
...
void CalibrationUtils::findTriangleWithin(vector2df pt, triangle* t, triangle* ts)
{
    ...
    */
    ...
    ts->s1 = new calibrationPoint(vector2df(0,0),-1);
    ts->s2 = new calibrationPoint(vector2df(0,0),-1);
    ts->s3 = new calibrationPoint(vector2df(0,0),-1);
	for (int i=0; i<cameraPoints->size(); i++)
    {
        distance = cameraPoints->at(i).coord.getDistanceFrom(pt);
        if (distance < value[0])
        {
            if ((t->s1->ind_circle != cameraPoints->at(i).ind_circle) || (t->s2->ind_circle != cameraPoints->at(i).ind_circle))
            {
                value[2] = value[1];
                t->s3=t->s2;
                ts->s3=ts->s2;
        ...
        else if ((distance < value[2]) && ((cameraPoints->at(i).ind_circle != t->s1->ind_circle) || (cameraPoints->at(i).ind_circle != t->s2->ind_circle)))
        {
